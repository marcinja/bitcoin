...
BOOST_FIXTURE_TEST_CASE(addrindex_initial_sync_and_spends, TestChain100Setup)
{
    ...
    for (int i = 0; i < 10; i++) {
        std::vector<std::pair<COutPoint, std::pair<CTransactionRef, uint256>>> spends;
        std::vector<std::pair<COutPoint, std::pair<CTransactionRef, uint256>>> creations;
        if (!addr_index.FindTxsByScript(script_pub_keys[i], spends, creations)) {
            BOOST_ERROR("FindTxsByScript failed");
        }

        // The coinbase transactions have the same scriptPubKey in their output.
        BOOST_CHECK_EQUAL(spends.size(), 0);
        BOOST_CHECK_EQUAL(creations.size(), 1);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (i >= 5) {
            ...
            BOOST_CHECK_EQUAL(creations3.size(), 2);
        }
    ...
    if (!addr_index.FindTxsByScript(coinbase_script_pub_key, spends4, creations4)) {
        BOOST_ERROR("FindTxsByScript failed");
    }
    BOOST_CHECK_EQUAL(spends4.size(), 10 + 5);
    BOOST_CHECK_EQUAL(creations4.size(), 100 + 20 + 2);
}
