...
BOOST_FIXTURE_TEST_CASE(addr_index_initial_sync_and_spends, TestChain100Setup)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (!addr_index.FindTxsByScript(script_pub_keys[i], spends3, creations3)) {
            BOOST_ERROR("FindTxsByScript failed");
        }

        // The coinbase transactions have the same scriptPubKey in their output.
        BOOST_CHECK_EQUAL(spends3.size(), 0);
        BOOST_CHECK_EQUAL(creations3.size(), 1);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (!addr_index.FindTxsByScript(script_pub_keys[i], spends4, creations4)) {
            BOOST_ERROR("FindTxsByScript failed");
        }

        // The coinbase transactions have the same scriptPubKey in their output.
        BOOST_CHECK_EQUAL(spends4.size(), 0);

        // Expect 2 transactions for those sent to twice, 1 for the rest.
        if (i >= 5) {
            BOOST_CHECK_EQUAL(creations4.size(), 1);
        } else {
            BOOST_CHECK_EQUAL(creations4.size(), 2);
        }
    ...
    if (!addr_index.FindTxsByScript(coinbase_script_pub_key, spends5, creations5)) {
        BOOST_ERROR("FindTxsByScript failed");
    }
    BOOST_CHECK_EQUAL(spends5.size(), 10 + 5);
    BOOST_CHECK_EQUAL(creations5.size(), 100 + 20 + 2);
