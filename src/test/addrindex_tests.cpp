...
// Creates a small reorg to generate them. Also tests that spending addresses show up (using tx_index).
BOOST_FIXTURE_TEST_CASE(addrindex_reorgs, TestChain100Setup)
{
    ...
    // Allow txindex to catch up with the block index, after which point it will be
    // processing ValidationInterface subscriber events.
    constexpr int64_t timeout_ms = 10 * 1000;
    int64_t time_start = GetTimeMillis();
